...
ID3D11Buffer *gpID3D11Buffer_Pyramid_VertexBuffer_Texture = NULL;

//rectangle vbos
ID3D11Buffer *gpID3D11Buffer_Cube_VertexBuffer_Position = NULL;
ID3D11Buffer *gpID3D11Buffer_Cube_VertexBuffer_Texture = NULL;
...
HRESULT initialize(void)
{
	...
	//create vertex texture color buffer
	D3D11_BUFFER_DESC bufferDescTexture;
	ZeroMemory(&bufferDescTexture, sizeof(D3D11_BUFFER_DESC));
	bufferDescTexture.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescTexture.ByteWidth = sizeof(float) * ARRAYSIZE(pyramid_texcoord);
	bufferDescTexture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescTexture.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescTexture, NULL, &gpID3D11Buffer_Pyramid_VertexBuffer_Texture);
	if (FAILED(hr))
	{
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubResourceForTexture;
	ZeroMemory(&mappedSubResourceForTexture, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Pyramid_VertexBuffer_Texture, NULL,
		D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubResourceForTexture);
	memcpy(mappedSubResourceForTexture.pData, pyramid_texcoord, sizeof(pyramid_texcoord));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_Pyramid_VertexBuffer_Texture, NULL);
	...
	bufferDescTexRect.ByteWidth = sizeof(float) * ARRAYSIZE(cube_texcoord);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescTexRect,
		NULL,
		&gpID3D11Buffer_Cube_VertexBuffer_Texture);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Cube_VertexBuffer_Texture,
		NULL,
		D3D11_MAP_WRITE_DISCARD, 
		NULL,
		&mappedSubResourceForTexRect);
	memcpy(mappedSubResourceForTexRect.pData, cube_texcoord, sizeof(cube_texcoord));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_Cube_VertexBuffer_Texture, NULL);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateSamplerState() Failed For Pyramid Texture.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateSamplerState() Succeeded For Pyramid Texture.\n");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateSamplerState() Failed For Cube Texture.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateSamplerState() Succeeded For Cube Texture.\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Pyramid_VertexBuffer_Texture, &stride, &offset);
	...
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Cube_VertexBuffer_Texture, &stride, &offset);
