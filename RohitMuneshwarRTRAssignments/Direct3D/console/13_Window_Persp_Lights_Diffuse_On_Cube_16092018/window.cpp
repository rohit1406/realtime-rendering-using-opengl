...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDescNormalRect;
	ZeroMemory(&bufferDescNormalRect, sizeof(D3D11_BUFFER_DESC));
	bufferDescNormalRect.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescNormalRect.ByteWidth = sizeof(float) * ARRAYSIZE(cube_normals);
	bufferDescNormalRect.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescNormalRect.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescNormalRect,
		NULL,
		&gpID3D11Buffer_Cube_VertexBuffer_Normals);
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubResourceForNormalsRect;
	ZeroMemory(&mappedSubResourceForNormalsRect, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Cube_VertexBuffer_Normals,
		NULL,
		D3D11_MAP_WRITE_DISCARD, 
		NULL,
		&mappedSubResourceForNormalsRect);
	memcpy(mappedSubResourceForNormalsRect.pData, cube_normals, sizeof(cube_normals));
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Cube_VertexBuffer_Normals, &stride, &offset);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_Cube_VertexBuffer_Normals)
	{
		gpID3D11Buffer_Cube_VertexBuffer_Normals->Release();
		gpID3D11Buffer_Cube_VertexBuffer_Normals = NULL;
	}
