...
ID3D11Buffer *gpID3D11Buffer_Tri_VertexBuffer_Position = NULL;
ID3D11Buffer *gpID3D11Buffer_Tri_VertexBuffer_Color = NULL;

//rectangle vbos
ID3D11Buffer *gpID3D11Buffer_Rect_VertexBuffer_Position = NULL;
ID3D11Buffer *gpID3D11Buffer_Rect_VertexBuffer_Color = NULL;
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL,
		&gpID3D11Buffer_Tri_VertexBuffer_Position);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Tri_VertexBuffer_Position, NULL, D3D11_MAP_WRITE_DISCARD,
		NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices_position, sizeof(vertices_position));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_Tri_VertexBuffer_Position, NULL);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescCol, NULL, &gpID3D11Buffer_Tri_VertexBuffer_Color);
	if (FAILED(hr))
	{
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Tri_VertexBuffer_Color, NULL, 
	...
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_Tri_VertexBuffer_Color, NULL);
	...
	bufferDescRect.ByteWidth = sizeof(float) * ARRAYSIZE(vertices_position_rect);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescRect, 
		NULL,
		&gpID3D11Buffer_Rect_VertexBuffer_Position);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Rect_VertexBuffer_Position,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubResourceRect);
	memcpy(mappedSubResourceRect.pData, vertices_position_rect, sizeof(vertices_position_rect));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_Rect_VertexBuffer_Position, NULL);
	...
	bufferDescColRect.ByteWidth = sizeof(float) * ARRAYSIZE(vertices_color_rect);
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescColRect,
		NULL,
		&gpID3D11Buffer_Rect_VertexBuffer_Color);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_Rect_VertexBuffer_Color,
		NULL,
		D3D11_MAP_WRITE_DISCARD, 
		NULL,
		&mappedSubResourceForColorRect);
	memcpy(mappedSubResourceForColorRect.pData, vertices_color_rect, sizeof(vertices_color_rect));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_Rect_VertexBuffer_Color, NULL);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_Tri_VertexBuffer_Position, &stride, &offset);

	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Tri_VertexBuffer_Color, &stride, &offset);
	...
	/***************Draw Rectangle Starts*******************/
	//select which vertex buffer to display
	stride = sizeof(float) * 3;
	offset = 0;

	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_Rect_VertexBuffer_Position, &stride, &offset);

	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Rect_VertexBuffer_Color, &stride, &offset);
	//select geometry primitive
	gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	worldMatrix = XMMatrixTranslation(1.5f, 0.0f, 6.0f);
	rotationMatrix = XMMatrixRotationX(XMConvertToRadians(gfSquareAngleRotation));
	worldMatrix = rotationMatrix * worldMatrix;
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_Tri_VertexBuffer_Position)
	{
		gpID3D11Buffer_Tri_VertexBuffer_Position->Release();
		gpID3D11Buffer_Tri_VertexBuffer_Position = NULL;
	}
	if (gpID3D11Buffer_Tri_VertexBuffer_Color)
	{
		gpID3D11Buffer_Tri_VertexBuffer_Color->Release();
		gpID3D11Buffer_Tri_VertexBuffer_Color = NULL;
	}

	if (gpID3D11Buffer_Rect_VertexBuffer_Position)
	{
		gpID3D11Buffer_Rect_VertexBuffer_Position->Release();
		gpID3D11Buffer_Rect_VertexBuffer_Position = NULL;
	}
	if (gpID3D11Buffer_Rect_VertexBuffer_Color)
	{
		gpID3D11Buffer_Rect_VertexBuffer_Color->Release();
		gpID3D11Buffer_Rect_VertexBuffer_Color = NULL;
	}
