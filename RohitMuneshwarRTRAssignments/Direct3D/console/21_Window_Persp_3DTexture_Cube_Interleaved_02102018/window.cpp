...
//rectangle vbos
ID3D11Buffer *gpID3D11Buffer_vertexBuffer_cube = NULL; //vbo
...
HRESULT initialize(void)
{
	...
	float cube_vcnt[] =
	{
		//v1     v2     v3       c1    c2    c3      n1     n2    n3     t1     t2 
		// SIDE 1 ( TOP )
		// triangle 1
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_vcnt);
	...
	hr = gpID3D11Device->CreateBuffer(
		...
		&gpID3D11Buffer_vertexBuffer_cube//ppBuffer
	);
	...
	if (FAILED(hr)) {
		...
		log("Success: gpID3D11Device->CreateBuffer for cube vertices poaition.\n");
	}
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_vertexBuffer_cube,//pResource
		NULL,//SubResource
	...
	inputElementDesc[1].InputSlot = 1; // like VDG_ATTRIBUTE_COLOR
	...
	inputElementDesc[2].InputSlot = 2; // like VDG_ATTRIBUTE_NORMAL
	...
	inputElementDesc[3].InputSlot = 3; // like VDG_ATTRIBUTE_TEXTURE0
	...
	gpID3D11Device->CreateInputLayout(
		...
		_ARRAYSIZE(inputElementDesc),//NumElements position and texcoord
	...
	gpID3D11DeviceContext->VSSetConstantBuffers(0,1,
		&gpID3D11Buffer_ConstanctBuffer);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(
		...
		1,//NUmBuffer
		&gpID3D11Buffer_vertexBuffer_cube,//ppvertexBuffer
		&stride,//pstride
		&offset//pOffset
	);
	...
	gpID3D11DeviceContext->IASetVertexBuffers(
		2,//StartSlot VDG_Attribute_normal
		1,//NUmBuffer
		&gpID3D11Buffer_vertexBuffer_cube,//ppvertexBuffer
		&stride,//pstride
		&offset//pOffset
	...
	gpID3D11DeviceContext->IASetVertexBuffers(
		3,//StartSlot VDG_Attribute_texcoord0
		1,//NUmBuffer
		&gpID3D11Buffer_vertexBuffer_cube,//ppvertexBuffer
		&stride,//pstride
		&offset//pOffset
	...
	gpID3D11DeviceContext->PSSetShaderResources(
		0,//start slot
		1,//NumViews
		&gpID3D11ShaderResourceView_Texture_Cube//ppShaderResourceView
	);
	gpID3D11DeviceContext->PSSetSamplers(
		0,//start slot
		1,//NumSampler
		&gpID3D11SamplerState_Texture_Cube//ppSamplerState 
	);
	...
	rotationMatrix = XMMatrixRotationX(XMConvertToRadians(gAngleCube));
	rotationMatrix *= XMMatrixRotationY(XMConvertToRadians(gAngleCube));
	rotationMatrix *= XMMatrixRotationZ(XMConvertToRadians(gAngleCube));
	translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 5.0f); // Z is + inword
