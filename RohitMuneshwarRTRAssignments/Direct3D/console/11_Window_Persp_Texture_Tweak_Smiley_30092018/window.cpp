...
HRESULT initialize(void)
{
	...
	bufferDescRect.ByteWidth = sizeof(float) * ARRAYSIZE(squareVertices);
	...
	memcpy(mappedSubResourceRect.pData, squareVertices, sizeof(squareVertices));
...
void twickTexCoordsData() {
	D3D11_BUFFER_DESC bufferDesc;
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(square1Texcoords);
	...
	HRESULT hr = gpID3D11Device->CreateBuffer(
		...
		&gpID3D11Buffer_Smiley_VertexBuffer_Texture//ppBuffer
	);

	if (FAILED(hr)) {
		log("Failed: gpID3D11Device->CreateBuffer for square vertices texcoord.\n");
	}
	else {
		log("Success: gpID3D11Device->CreateBuffer for square vertices texcoord.\n");
	}

	D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_Smiley_VertexBuffer_Texture,//pResource
		NULL,//SubResource
	...
	if (gNumKeyPressed == 0) {
		memcpy(
			...
			square1Texcoords,//source
			sizeof(square1Texcoords)//size
		);
	}
	else if (gNumKeyPressed == 1) {
		memcpy(
			mappedSubresource.pData,//dest
			square2Texcoords,//source
			sizeof(square2Texcoords)//size
		);
	}
	else if (gNumKeyPressed == 2) {
		memcpy(
			mappedSubresource.pData,//dest
			square3Texcoords,//source
			sizeof(square3Texcoords)//size
		);
	}
	else if (gNumKeyPressed == 3) {
		memcpy(
			mappedSubresource.pData,//dest
			square4Texcoords,//source
			sizeof(square4Texcoords)//size
		);
	}
	else if (gNumKeyPressed == 4) {
		memcpy(
			mappedSubresource.pData,//dest
			square5Texcoords,//source
			sizeof(square5Texcoords)//size
		);
	}
