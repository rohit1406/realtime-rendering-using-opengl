...
GLuint gVbo;
GLuint gMVPUniform;
GLuint gtexture_sampler_uniform0;
...
void initialize(){
	...
	const GLchar *vertexShaderSourceCode =
				...
				"out vec2 out_tex_coord0;"\
				"out vec2 out_tex_coord1;"\
				"uniform float time;"\
				"void main(void)						"\
				"{										"\
				"const mat2 m = mat2(vec2(cos(time), sin(time)), vec2(-sin(time), cos(time)));"\
				"out_tex_coord0=vTexCoord * m;"\
				"out_tex_coord1=vTexCoord * transpose(m);"\
				"gl_Position = vec4(vPosition, 0.5, 0.1);	"\
	...
	const GLchar *fragmentShaderSourceCode =
				"#version 440" \
				"\n" \
				"uniform sampler2D u_sampler_tex0;"\
				"uniform sampler2D u_sampler_tex1;"\
				"in vec2 out_tex_coord0;"\
				"in vec2 out_tex_coord1;"\
				"out vec4 FragColor;" \
				"void main(void)" \
				"{" \
				"FragColor=texture(u_sampler_tex0, out_tex_coord0) + texture(u_sampler_tex1, out_tex_coord1);" \
				"}";
	...
	gtexture_sampler_uniform0 = glGetUniformLocation(gShaderProgramObject, "u_sampler_tex0");
	gtexture_sampler_uniform1 = glGetUniformLocation(gShaderProgramObject, "u_sampler_tex1");
...
void display(){
	...
	glUniform1i(gtexture_sampler_uniform0, 0);
	glActiveTexture(0);
	
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, texture_checkerboard);
	glUniform1i(gtexture_sampler_uniform1, 1);
	glActiveTexture(0);
