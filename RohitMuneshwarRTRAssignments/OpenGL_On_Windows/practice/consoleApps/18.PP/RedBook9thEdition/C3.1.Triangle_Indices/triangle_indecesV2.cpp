...
void initialize(){
	...
	glVertexAttribPointer(VDG_ATTRIBUTE_VERTEX, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
...
void display(){
	...
	//translate
	modelViewMatrix = translate(-3.0f,0.0f,-6.0f);
	...
	//0 - starting at offset zero wrt buffer offset we set with glVertexAttribPointer
	...
	//draw, either by glDrawTriangles() or glDrawArrays() or glDrawElements()
	//defines sequence of geometric primitives using 3 number of vertices, whose indices are stored in the buffer bound to the GL_ELEMENT_ARRAY_BUFFER buffer
	//indices represents an offset, in bytes, into the element array buffer where the indices begins
