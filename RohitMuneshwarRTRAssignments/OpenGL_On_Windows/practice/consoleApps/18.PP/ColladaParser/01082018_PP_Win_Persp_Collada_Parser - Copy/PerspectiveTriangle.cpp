...
void initialize(){
	void resize(int,int);
	void uninitialize(void);
	struct ColladaData loadColladaFile(char *);
	...
	struct ColladaData colladaDataContents = loadColladaFile("colladaFiles/Iron_Man.dae");
	...
	//fprintf(g_fp_logfile, "Collada File Loaded With Objects: %d - positin size-%d\n", num_objects,geom_vec[0].map["POSITION"].size);
	...
	*/
	...
	glBufferData(GL_ARRAY_BUFFER, gNormalDataSize * 3 * sizeof(GL_FLOAT), &colladaDataContents.meshData.normalInfo.vec_normal[0], GL_STATIC_DRAW);
...
struct ColladaData loadColladaFile(char *fileName)
{
	return loadDaeData(fileName);
...
void display(){
	...
	glDrawArrays(GL_TRIANGLES, 0, gPositionDataSize);
	//glDrawElements(GL_TRIANGLES, geom_vec[0].index_count, 
      //             GL_UNSIGNED_SHORT, geom_vec[0].indices);
