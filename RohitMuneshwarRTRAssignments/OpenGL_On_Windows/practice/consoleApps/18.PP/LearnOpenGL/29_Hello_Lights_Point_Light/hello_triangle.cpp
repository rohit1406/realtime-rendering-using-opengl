...
vec3 lightPos = vec3(1.3f, 0.8f, -6.0f);
...
void initialize(){
	...
	const GLchar *fragmentShaderSourceCode =
				...
				"void main(void)" \
				"{" \
				"float distanceOfLightSourceFromFragment = length(u_light_position - out_fragPos);"\
				"float attenuation = 1.0 / (u_constant + u_linear * distanceOfLightSourceFromFragment + u_quadratic * (distanceOfLightSourceFromFragment * distanceOfLightSourceFromFragment));"\
...
void display(){
	...
	glUniform1f(gLinearUniform, 0.14f);
	glUniform1f(gQuadraticUniform, 0.07f);
