...
GLuint gObjectColorUniform, gLightColorUniform, gLightPositionUniform,gLightDirectionUniform,gLightCutOffUniform, gViewerPositionUniform;

//light component uniforms
...
//to calculate attenuation of light
...
vec3 lightPos = vec3(1.3f, 0.8f, -5.0f);
...
const vec3 cubePositions[]=
	{
		vec3(0.0f, 0.0f, -5.0f),
		vec3(2.0f, 2.0f, -15.0f),
		vec3(-2.0f, -1.8f, -5.5f),
		vec3(-2.8f, -2.0f, -12.3f),
		vec3(2.0f, -0.4f, -3.5f),
		vec3(-3.7f, 2.2f, -7.5f),
		vec3(1.0f, -2.0f, -6.5f),
		vec3(3.4f, 1.0f, -7.5f),
...
void initialize(){
	...
	const GLchar *fragmentShaderSourceCode =
				...
				"float theta = dot(lightDir, normalize(-u_light_direction));"\
				"vec4 color;"\
				"if(theta > u_cutOff)"\
				"{"\
				""\
				...
				"else"\
				"{"\
				"color = vec4(u_light_ambient * vec3(texture(u_material_diffuse, out_tex_coords)), 1.0);"\
				"}"\
	...
	gLightDirectionUniform = glGetUniformLocation(gShaderProgramObject, "u_light_direction");
	gLightCutOffUniform = glGetUniformLocation(gShaderProgramObject, "u_cutOff");
	...
	gConstantUniform = glGetUniformLocation(gShaderProgramObject, "u_constant");
	gLinearUniform = glGetUniformLocation(gShaderProgramObject, "u_linear");
	gQuadraticUniform = glGetUniformLocation(gShaderProgramObject, "u_quadratic");
...
void display(){
	...
	//scale light source
	rotationMatrix = rotate(rotationAngle, 1.0f, 0.3f, 0.5f);
	...
	glUniform3fv(gLightAmbientUniform, 1, vec3(0.1f, 0.1f, 0.1f));
	glUniform3fv(gLightDiffuseUniform, 1, vec3(0.8f, 0.8f, 0.8f));
	glUniform3fv(gLightSpecularUniform, 1, vec3(1.0f, 1.0f, 1.0f));
	glUniform3fv(gLightPositionUniform, 1, lightPos);
	glUniform3fv(gLightDirectionUniform, 1, vec3(0.0f, 0.0f, -3.0f));
	glUniform1f(gLightCutOffUniform, radians(30.5f));
	...
	glUniform1f(gLinearUniform, 0.09f);
	glUniform1f(gQuadraticUniform, 0.032f);
