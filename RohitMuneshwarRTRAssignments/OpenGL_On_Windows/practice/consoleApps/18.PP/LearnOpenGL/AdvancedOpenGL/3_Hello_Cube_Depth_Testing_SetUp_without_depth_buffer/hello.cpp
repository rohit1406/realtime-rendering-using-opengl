...
void initialize(){
	...
	//depth test to do
	//GL_ALWAYS - depth test always passes - same behaviour we get when we don't enable depth testing
	//so the fragments that are drawn last are rendered in front of the fragments that were drawn before. Since we have drawn the floor plane as the last, the floor fragments overwrite each of the containers fragment
