...
void initialize(){
	...
	glStencilFunc(GL_EQUAL, //stencil test function
				1, //stencil buffers content is compared to this value
				0x00 //mask that is ANDed with both the reference values and the stored stencil value before the test compares them
				);
	...
	glStencilOp(GL_KEEP, //sfail - action to take if test fails
				...
				GL_KEEP //dppass - action to take if both the test passes
				);
