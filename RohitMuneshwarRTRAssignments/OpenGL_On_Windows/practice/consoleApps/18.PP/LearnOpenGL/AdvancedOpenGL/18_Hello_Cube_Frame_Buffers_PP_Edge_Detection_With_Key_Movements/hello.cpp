...
/*
	...
	pixel that multiplies surrounding pixel values by its kernel values and add
	them all together to form a single value

	kernel that produces edge detection effect is as mentioned in the fragment shader
	
	see the change in fragment shader
*/
#include<windows.h>
...
//textures
GLuint container_texture, gFloorTexture, gTextureGrass;
...
void display(){
	...
	//set color to white - not really necessary actually, since wen won't be able to see behind the quad anyways
	glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
	glClear(GL_COLOR_BUFFER_BIT);
	
	glUseProgram(gQuadShaderProgramObject);
...
void createVertexShader()
{
	...
	gQuadVertexShaderObject = gVertexShaderObject;
}
