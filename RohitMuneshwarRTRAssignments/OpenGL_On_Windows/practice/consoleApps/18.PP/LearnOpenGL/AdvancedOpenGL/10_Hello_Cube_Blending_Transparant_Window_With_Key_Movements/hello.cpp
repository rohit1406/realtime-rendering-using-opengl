...
//transparent object
GLuint gVaoTransparent;
GLuint gVboTransparent;
...
GLuint transparentObjectsCount = 2;
const vec3 transparentObjectsPositions[]=
	{
		vec3(0.3f, 0.0f, -4.2f),
		vec3(-1.3, 0.0f, -3.0f)
...
vec3 cameraPos = vec3(camX, 2.0f, camZ); //position vector
...
void initialize(){
	...
	const GLchar *fragmentShaderSourceCode =
				...
				"void main(void)" \
				"{" \
				"vec4 texColor = texture(u_sampler_texture, out_tex_coords);"\
				"FragColor=texColor;" \
				"}";
	...
	//transparent object vao configuration
	glGenVertexArrays(1, &gVaoTransparent);
	glBindVertexArray(gVaoTransparent);
	
	//transparent object vbo
	glGenBuffers(1, &gVboTransparent);
	glBindBuffer(GL_ARRAY_BUFFER, gVboTransparent);
	glBufferData(GL_ARRAY_BUFFER, sizeof(grassVertices), grassVertices, GL_STATIC_DRAW);
	glVertexAttribPointer(VDG_ATTRIBUTE_VERTEX, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void *)0);
	glEnableVertexAttribArray(VDG_ATTRIBUTE_VERTEX);
	glVertexAttribPointer(VDG_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void *) (3 * sizeof(float)));
	...
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
	//transparent object vao configuration ends
	...
	loadTextureWithAlpha(&gTextureTransperant, "blending_transparent_window.png");
...
void display(){
	...
	//bind transparent object vao
	glBindVertexArray(gVaoTransparent);
	//bind texture
	glBindTexture(GL_TEXTURE_2D, gTextureTransperant);
	for(unsigned int i=0;i<transparentObjectsCount;i++){
	//translate
	modelMatrix = mat4::identity();
	//rotationMatrix = mat4::identity();
	modelMatrix = translate(transparentObjectsPositions[i]);
	//rotationMatrix = rotate(180.0f, 0.0f, 0.0f, 1.0f);
	//modelMatrix = modelMatrix * rotationMatrix;
	glUniformMatrix4fv(gModelUniform, 1, GL_FALSE, modelMatrix);
	glDrawArrays(GL_TRIANGLES, 0, 6);
...
void uninitialize(){
	...
	if(gVaoTransparent)
	{
		glDeleteVertexArrays(1, &gVaoTransparent);
		gVaoTransparent = 0;
	}
	...
	if(gVboTransparent)
	{
		glDeleteBuffers(1, &gVboTransparent);
		gVboTransparent = 0;
	}
	...
	if (gTextureTransperant)
	{
		glDeleteTextures(1, &gTextureTransperant);
		gTextureTransperant = 0;
	}
