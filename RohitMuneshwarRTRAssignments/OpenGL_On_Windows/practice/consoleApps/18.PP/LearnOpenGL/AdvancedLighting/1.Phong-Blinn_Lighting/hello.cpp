...
GLuint gUniformTextureBricksSampler, gUniformLightColor, gUniformLightPos, gUniformViewerPos, gUniformIsBlinn;
...
void initialize(){
	...
	gLightColor = vec3(1.0f,1.0f,1.0f);
	gLightPos = vec3(0.0f, 0.0f, 0.0f);
	gCameraPos = vec3(0.0f, 0.0f, -3.0f);
	gIsBPress = false;
	//resize
...
void display(){
	...
	//rotate
	//gRotationTransformationMatrix = rotate(90.0f, 1.0f, 0.0f, 0.0f);
	...
	glUniform3fv(gUniformLightColor, 1, gLightColor);
	glUniform3fv(gUniformLightPos, 1, gLightPos);
	glUniform3fv(gUniformViewerPos, 1, gCameraPos);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gTextureBricks);
	glUniform1i(gUniformTextureBricksSampler, 0);
	if (gIsBPress == true) 
	{
		glUniform1i(gUniformIsBlinn, 1);
	}
	else
	{
		glUniform1i(gUniformIsBlinn, 0);
	}
...
void createVertexShader(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"out_fragment_pos = vec3(u_model_matrix * vec4(vPosition, 1.0));"\
		"}										";
...
void createFragmentShader(void)
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"in vec3 out_normals;"\
		"in vec3 out_fragment_pos;"\
		"uniform sampler2D u_bricks_texture;"\
		...
		"vec3 lightDir = normalize(u_light_pos - out_fragment_pos);"\
		"float diff = max(dot(norm, lightDir), 0.0);"\
		"vec3 diffuse = diff * u_light_color;"\
		"vec3 reflectionDir = reflect(-lightDir, norm);"\
		"vec3 viewDir = normalize(u_viewer_pos - out_fragment_pos);"\
		...
		"spec = pow(max(dot(out_normals, halfwayDir), 0.0), specularShininess);"\
		"specular = spec * u_light_color;"\
		"}"\
		"else"\
		"{"\
		
		"spec = pow(max(dot(viewDir, reflectionDir), 0.0), specularShininess);"\
		"specular = specularStrength * spec * u_light_color;"\
		"}"\
		
		
		"vec3 result = (ambient + diffuse + specular) * vec3(texture(u_bricks_texture, out_tex_coords));"\
...
void createShaderProgram(void)
{
	...
	glBindAttribLocation(gShaderProgramObject, VDG_ATTRIBUTE_NORMAL, "vTexCoords");
...
void getUniformLocations(void)
{
	...
	gUniformTextureBricksSampler = glGetUniformLocation(gShaderProgramObject, "u_bricks_texture");
	gUniformLightColor = glGetUniformLocation(gShaderProgramObject, "u_light_color");
	gUniformLightPos = glGetUniformLocation(gShaderProgramObject, "u_light_pos");
	gUniformViewerPos = glGetUniformLocation(gShaderProgramObject, "u_viewer_pos");
	gUniformIsBlinn = glGetUniformLocation(gShaderProgramObject, "u_blinn");
}
...
void loadTexture(GLuint *texture, char* path)
{
	...
	if (data) {
		...
		glTexImage2D(GL_TEXTURE_2D, //texture target: this operation will generate texture on currently bound texture object at the same target - it won't affect GL_TEXTURE_1D or GL_TEXTURE_3D
			...
			format, //in what kind of format we want to store the texture
