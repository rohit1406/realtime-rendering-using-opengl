...
-(void)prepareOpenGL
{
    ...
    //load textures
    gTexturePyramid=[self loadTextureFromBMPFile:"Stone.bmp"];
    gTextureCube=[self loadTextureFromBMPFile:"Vijay_Kundali.bmp"];
...
-(void)drawView
{
    ...
    //bind texture
    glBindTexture(GL_TEXTURE_2D, gTextureCube);
...
-(void) dealloc
{
    ...
    if(gTexturePyramid)
    {
        glDeleteTextures(1, &gTexturePyramid);
        gTexturePyramid = 0;
    }
    
    if(gTextureCube)
    {
        glDeleteTextures(1, &gTextureCube);
        gTextureCube=0;
    }
